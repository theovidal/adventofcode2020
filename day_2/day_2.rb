...
inputs.each do |input|
  ...
  boundary1, boundary2 = limit.split("-")
  boundary1 = boundary1.to_i
  boundary2 = boundary2.to_i
  letter.chop!
  
  # Part 1 - Boundary 1 is the minimum amount of letters in the password, and Boundary 2 the max
  condensed_password = password.gsub(/[^#{letter}]/, '')
  part_one += 1 if condensed_password.length >= boundary1 && condensed_password.length <= boundary2
  ...
  part_two += 1 if (!letter1 && letter2) || (letter1 && !letter2)
end
